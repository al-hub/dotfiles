...
설치 시 사용한 버전 또는 branch는 `~/.dotfiles-install/version`에 기록됩니다. 새 버전을 배포할 때는 해당 커밋에 `v0.6.1` 같은 git tag를 만들고, 사용자가 `--v v0.6.1`로 설치할 수 있게 합니다. 성능 비교 리포트는 `tests/profile-reports/`에 버전별로 보관합니다.

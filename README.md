...
설치 시 사용한 버전 또는 branch는 `~/.dotfiles-install/version`에 기록됩니다. 현재 sidebar 최적화 기준은 `v0.6.7`이며, 사용자는 `--v v0.6.7`로 고정 설치할 수 있습니다. 성능 비교 리포트는 `tests/profile-reports/`에 버전별로 보관합니다. v0.6.7은 lifecycle/invariant와 launcher 내부 latency 목표를 통과했으며, 외부 key latency 48ms(목표 40ms)는 tmux/PTY 관측 과제로 추적합니다.

...
- 기본 설치는 master 최신 기준이며, 안정 버전은 `v0.1`부터 `install.sh --v v0.1`로 tag 기준 설치할 수 있게 준비했습니다. v0.6~v0.6.6을 이전 기준으로 보존하고, 현재 안정 기준은 v0.6.7(v6.7)입니다. launcher 내부 latency는 목표를 통과했으며 외부 key latency 40ms 목표 미달은 tmux/PTY 관측 과제로 추적합니다.

...
- 기본 설치는 master 최신 기준이며, 안정 버전은 `v0.1`부터 `install.sh --v v0.1`로 tag 기준 설치할 수 있게 준비했습니다. 현재 작업은 v0.5로 정리했고, `v0.5` tag를 별도 릴리스 기준으로 사용합니다.
- 기본 enabled 설치 항목은 사용자에게 `tmux`만 보이며, `tmux-session-launcher`, `tmux-zshrc`, `urxvt-resize-font`, `tmux-xresources`, `tmux-theme-picker`는 hidden dependency로 함께 설치됩니다.

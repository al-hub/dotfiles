#!/usr/bin/env bash
# Standalone baseline plus extended navigation/lifecycle scenarios.
set -euo pipefail
...
{
    local target="$1"
    wait_for_pane_text "$auto_sidebar" "^>\\*? +$target"
}
...
auto_nav_move()
{
    ...
    auto_nav_wait "$target" || auto_status=FAIL
    observed="$(now_ms)"
...
tmuxc switch-client -t '=nav-01'
auto_sidebar="$(open_sidebar_direct nav-01)" || auto_status=FAIL
auto_start="$(now_ms)"
auto_nav_wait nav-01 || auto_status=FAIL

for auto_index in $(seq 2 10); do
    auto_nav_move j "nav-$(printf '%02d' "$auto_index")" down
done
for auto_index in $(seq 9 -1 1); do
    auto_nav_move k "nav-$(printf '%02d' "$auto_index")" up
done

auto_burst_start="$(now_ms)"
for auto_ignored in 1 2 3 4 5; do
    tmuxc send-keys -t "$auto_sidebar" j
done
auto_nav_wait nav-06 || auto_status=FAIL
printf 'AUTO_SCENARIO\tfast_burst_j_5\tnav-06\t%s\n' "$(( $(now_ms) - auto_burst_start ))"

sleep 5.2
auto_nav_move j nav-07 periodic_refresh_collision

tmuxc resize-pane -t "$auto_sidebar" -x 20 || auto_status=FAIL
sleep 0.2
tmuxc resize-pane -t "$auto_sidebar" -x 35 || auto_status=FAIL
auto_width="$(tmuxc display-message -p -t "$auto_sidebar" '#{pane_width}' 2>/dev/null || printf '0')"
[ "$auto_width" = 35 ] || auto_status=FAIL
printf 'AUTO_SCENARIO\tresize_20_to_35\tnav-07\t%s\n' "$auto_status"

auto_capture="$RUN_DIR/auto-sidebar.txt"
tmuxc capture-pane -p -t "$auto_sidebar" > "$auto_capture"
auto_cursor_count="$(grep -Ec '^>\\*? ' "$auto_capture" || true)"
[ "$auto_cursor_count" -eq 1 ] || auto_status=FAIL
...
[ "$layout_status" = PASS ] && [ "$grid_status" = PASS ] && [ "$auto_status" = PASS ]

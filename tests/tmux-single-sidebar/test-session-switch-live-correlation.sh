...
{
    local pane="$1" trace="$2"
    printf '%s\n' "$trace" | awk -v prefix="pane=$pane " '
        ...
        index($0, prefix "render.full.begin") { if (!geometry) count++; geometry=0 }
        END { print count + 0 }
    '
}
...
sample_transition()
{
    local iteration="$1" start_ms="$2" operation_id="$3" before_sidebar="$4" trace_before="$5"
    ...
    local before_pane before_pid current_pane current_pid
    before_pane="$(sidebar_field pane "$before_sidebar")"
    before_pid="$(sidebar_field pid "$before_sidebar")"
    ...
    for _ in $(seq 1 160); do
        ...
        full_render_count="$(non_geometry_render_count "$before_pane" "$recent_trace")"
        hook_event_count="$(printf '%s\n' "$recent_trace" | grep -Ec "pane=$before_pane .*transition\\.hook|pane=$before_pane .*sidebar\\.hook" || true)"
        ...
        if [ -n "$first_target_ms" ] && [ -n "$current_pane" ] &&
            [ "$current_pane" != absent ] && [ "$current_pane" != "$before_pane" ]; then
            identity_changed=1
        fi
        ...
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
            ...
            "$before_pane" "$before_pid" "$output_size" "$trace_size" "$sample_interval" "$full_render_count" "$hook_event_count" "$pane_state" "$sample_class" \
            >> "$SAMPLES_FILE"
        ...
        if [ -n "$first_target_ms" ] && [ "$current_pane" = "$before_pane" ] && [ "$current_pid" = "$before_pid" ]; then
            stable=$((stable + 1))
        else
            stable=0
        fi
...
select_session_by_name live-corr-1
prepare_topology || {

...
sub_p="$(tmux -L "$SOCKET" list-panes -t "$win_1" -F "#{pane_id}|#{@dotfiles_sidebar_subpane}" | awk -F "|" '$2 == "1" { print $1 }')"
...
sleep 0.5
sb_2="$(tmux -L "$SOCKET" list-panes -t "$win_2" -F "#{pane_id}|#{@dotfiles_sidebar_pane}" | awk -F "|" '$2 == "1" { print $1 }')"
...
sleep 0.5
sb_3="$(tmux -L "$SOCKET" list-panes -t "$win_3" -F "#{pane_id}|#{@dotfiles_sidebar_pane}" | awk -F "|" '$2 == "1" { print $1 }')"
...
# Step 6: In sess_3, toggle whole sidebar OFF with Ctrl+a s
bash "$SCRIPT_DIR/dist/tmux-session-launcher" --toggle-sidebar "$win_3"
sleep 0.5
count_sidebars="$(tmux -L "$SOCKET" list-panes -a -F "#{@dotfiles_sidebar_pane}" | grep -c "1" || true)"
[ "$count_sidebars" -eq 0 ] || { echo "FAIL: sidebars not closed"; exit 1; }

# Step 7: In sess_3, toggle whole sidebar ON with Ctrl+a s
bash "$SCRIPT_DIR/dist/tmux-session-launcher" --toggle-sidebar "$win_3"
sleep 0.5

sub_3_restored="$(sidebar_window_subpane "$win_3" || true)"
sub_1_check="$(sidebar_window_subpane "$win_1" || true)"
sub_2_check="$(sidebar_window_subpane "$win_2" || true)"

[ "$sub_3_restored" = "$sub_p" ] || { echo "FAIL: subpane not restored in active window win_3"; exit 1; }
[ -z "$sub_1_check" ] || { echo "FAIL: subpane wrongly attached to background win_1"; exit 1; }
[ -z "$sub_2_check" ] || { echo "FAIL: subpane wrongly attached to background win_2"; exit 1; }

h_3_restored="$(tmux -L "$SOCKET" display-message -p -t "$sub_3_restored" "#{pane_height}")"
[ "$h_3_restored" -eq 22 ] || { echo "FAIL: height is $h_3_restored, expected 22"; exit 1; }
